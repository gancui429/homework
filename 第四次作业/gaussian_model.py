...
# 替代 pytorch3d.ops.knn.knn_points 的函数
def knn_points(points1, points2, K):
    """
    替代 pytorch3d 的 knn_points 函数
    points1: (B, N, 3) 查询点
    points2: (B, M, 3) 参考点
    K: 最近邻数量
    返回: (dists, indices, None)
    """
    # 计算欧氏距离
    dists = torch.cdist(points1, points2)  # (B, N, M)
    
    # 获取最近的 K 个邻居
    dists, indices = torch.topk(dists, k=K, dim=-1, largest=False)
...
class GaussianModel(nn.Module):
    ...
    def compute_covariance(self) -> torch.Tensor:
        ...
        # Compute covariance: Σ = R * S * S^T * R^T = R * S^2 * R^T
        # Since S is diagonal, S * S^T = diag(s)^2
        S_squared = torch.diag_embed(scales ** 2)  # (N, 3, 3)
        Covs3d = torch.bmm(R, torch.bmm(S_squared, R.transpose(1, 2)))
